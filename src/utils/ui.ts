...
export function card(opts: {
  ...
  const head =
    opts.title || opts.subtitle
      ? `<div class="card-body border-b border-light-dark pb-4">
          ${opts.title ? `<h2 class="card-title">${opts.title}</h2>` : ''}
          ${opts.subtitle ? `<p class="card-subtitle mt-1">${opts.subtitle}</p>` : ''}
        </div>`
      : '';
  const bodyPad = opts.title || opts.subtitle ? 'p-[30px]' : 'card-body';
...
export function th(text: string, opts?: { align?: 'left' | 'right' | 'center' }): string {
  const align = opts?.align === 'right' ? ' text-right' : opts?.align === 'center' ? ' text-center' : '';
  return `<th scope="col" class="${align.trim()}">${text}</th>`;
}

export function dataTable(headers: string[], bodyRows: string, opts?: { class?: string }): string {
...
export function tableRow(cells: string[], opts?: { class?: string }): string {
  const cls = opts?.class ?? '';
  return `<tr class="${cls}">${cells.map((c) => `<td>${c}</td>`).join('')}</tr>`;
}

export function tableEmpty(colspan: number, message = 'Chưa có dữ liệu'): string {
...
export function tableActions(editOnclick?: string, deleteOnclick?: string): string {
  ...
  const del = deleteOnclick
    ...
    : '';
  if (!edit && !del) return '—';
  return `<div class="flex items-center gap-1">${edit}${del}</div>`;
}
...
export function formGroup(label: string, control: string, opts?: { required?: boolean }): string {
...
export function input(attrs: Record<string, string | boolean | undefined>): string {
...
export function textarea(attrs: Record<string, string | boolean | undefined>): string {
...
export function alert(type: 'error' | 'success' | 'warning' | 'info', message: string): string {
  const map = {
...
export function linkBtn(href: string, label: string, className = 'text-primary hover:underline cursor-pointer'): string {
